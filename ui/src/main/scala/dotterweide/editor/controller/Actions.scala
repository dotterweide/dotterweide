...
private class Actions(document: Document, terminal: Terminal, data: Data, adviser: Adviser,
                      formatter: Formatter, tabSize: Int, lineCommentPrefix: String, history: History)(implicit async: Async)
  ...
  val indentSelection   : Action = historical(new IndentSelection(document, terminal, tabSize = tabSize))
